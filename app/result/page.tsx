  ...
  return (
    <div className="bg-white rounded-xl border border-[#E5E7EB] overflow-hidden mb-6">
      ...
      >
        ...
        {isOpen ? <ChevronUp className="w-5 h-5 text-[#696A72]" /> : <ChevronDown className="w-5 h-5 text-[#696A72]" />}
      </button>
  ...
  return (
    <div className="flex flex-col items-center p-4 bg-gray-50 rounded-xl">
      <div className="relative w-20 h-20 mb-3">
        ...
        <span className="absolute inset-0 flex items-center justify-center text-2xl font-bold text-[#020617]">{score}</span>
      </div>
      <span className="text-sm text-[#696A72]">{label}</span>
    </div>
  );
}
      ...
      {phases.map((phase, index) => (
        <div key={phase.phase} className="flex gap-4 pb-6 last:pb-0">
          <div className="flex flex-col items-center">
            <div className="w-10 h-10 rounded-full bg-[#0000EE] text-white flex items-center justify-center font-bold text-sm">
              {phase.phase}
            </div>
            {index < phases.length - 1 && <div className="w-0.5 h-full bg-[#E5E7EB] mt-2" />}
          </div>
          <div className="flex-1">
            <div className="flex items-center justify-between mb-2">
              <h3 className="font-semibold text-[#020617]">{phase.name}</h3>
              <span className="text-sm text-[#696A72]">{phase.duration}</span>
            </div>
            <p className="text-sm text-[#0000EE] mb-3">{phase.costRange}</p>
            <div className="mb-3">
              <p className="text-xs text-[#696A72] mb-1">重点工作：</p>
              <div className="flex flex-wrap gap-2">
                {phase.focus.map((f, i) => (
                  <span key={i} className="px-2 py-1 bg-gray-100 rounded text-xs text-[#374151]">{f}</span>
                ))}
              </div>
            </div>
            <div>
              <p className="text-xs text-[#696A72] mb-1">关键任务：</p>
...
<head>
  ...
  <style>
    ...
    h1 { text-align: center; color: #0071E3; margin-bottom: 40px; }
    h2 { color: #1D1D1F; border-bottom: 2px solid #0071E3; padding-bottom: 10px; margin-top: 30px; }
    h3 { color: #020617; margin-top: 20px; }
    h4 { color: #0000EE; }
    p { margin: 10px 0; }
    ...
    .total-budget { font-size: 24px; font-weight: bold; color: #0071E3; text-align: right; }
    img { max-width: 100%; height: auto; border-radius: 8px; margin: 10px 0; }
    .image-caption { text-align: center; color: #6E6E73; font-size: 14px; margin-bottom: 20px; }
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md border-b border-[#E5E7EB]">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="text-xl font-semibold text-[#020617]">HomeGPT</span>
          </Link>
      ...
      <main className="pt-24 pb-20">
        <div className="max-w-6xl mx-auto px-6 py-12">
          ...
          <div className="text-center mb-12">
            <h1 className="text-3xl md:text-4xl font-bold text-[#020617] mb-4">
              HomeGPT 设计方案报告
            </h1>
            <p className="text-[#696A72]">
          ...
          <div className="bg-white rounded-xl border border-[#E5E7EB] p-6 mb-6">
            <div className="flex items-center justify-center gap-3 mb-2">
              <TrendingUp className="w-6 h-6 text-[#0000EE]" />
              <span className="text-lg font-medium text-[#020617]">综合评分</span>
            </div>
            <div className="text-center">
              <span className="text-6xl font-bold text-[#0000EE]">{fpAnalysis.overallScore}</span>
            ...
            <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <ScoreCard score={fpAnalysis.zoning.score} label="分区" color="#0000EE" />
              <ScoreCard score={fpAnalysis.utilization.score} label="利用率" color="#34C759" />
            </div>
          </div>

          <Section title="户型分析师" icon={<Layout className="w-5 h-5 text-[#0000EE]" />}>
            <div className="space-y-6">
              <div>
                <h3 className="font-semibold text-[#020617] mb-2">户型结构</h3>
              ...
              <div>
                <h3 className="font-semibold text-[#020617] mb-2 flex items-center gap-2">
                  <Sun className="w-4 h-4 text-[#FFB800]" />
                  采光分析
                </h3>
                <p className="text-[#374151] mb-3">{fpAnalysis.lighting.description}</p>
                <div className="mb-3">
                  <p className="text-sm text-[#696A72] mb-2">窗户分布：</p>
                ...
                <div>
                  <p className="text-sm text-[#696A72] mb-2">优化建议：</p>
              ...
              <div>
                ...
                <div className="mb-3">
                  <p className="text-sm text-[#696A72] mb-2">空气对流路径：</p>
                  <div className="flex flex-wrap gap-2">
                    {fpAnalysis.ventilation.airFlowPaths.map((path, i) => (
                      <span key={i} className="px-3 py-1 bg-blue-50 rounded-full text-sm text-blue-700">
                        {path}
                      </span>
                    ))}
                  </div>
              ...
              <div>
                <h3 className="font-semibold text-[#020617] mb-2 flex items-center gap-2">
                  <Layout className="w-4 h-4 text-[#0000EE]" />
                  动静分区
                </h3>
                <p className="text-[#374151] mb-3">{fpAnalysis.zoning.description}</p>
                <div className="grid grid-cols-2 gap-4 mb-3">
                  <div>
                    <p className="text-sm text-[#696A72] mb-2">公共区域：</p>
                    <div className="flex flex-wrap gap-2">
                      {fpAnalysis.zoning.publicAreas.map((area, i) => (
                        <span key={i} className="px-2 py-1 bg-gray-100 rounded text-xs text-[#374151]">{area}</span>
                      ))}
                    </div>
                  </div>
                  <div>
                    <p className="text-sm text-[#696A72] mb-2">私密区域：</p>
                    <div className="flex flex-wrap gap-2">
          ...
          <Section title="家庭生活规划师" icon={<Users className="w-5 h-5 text-[#0000EE]" />}>
            <div className="space-y-6">
              <div>
                <h3 className="font-semibold text-[#020617] mb-2">家庭画像</h3>
                <p className="text-[#374151]">{fpAnalysis2.familyProfile}</p>
              </div>

              <div>
                <h3 className="font-semibold text-[#020617] mb-2">生活方式分析</h3>
                <div className="space-y-3">
                  {fpAnalysis2.lifestyleAnalysis.dailyFlow.map((flow, i) => (
                    <div key={i} className="flex items-start gap-2 p-3 bg-gray-50 rounded-lg">
                      <Clock className="w-4 h-4 text-[#0000EE] mt-0.5" />
              ...
              <div>
                <h3 className="font-semibold text-[#020617] mb-2">日常活动模式</h3>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-[#E5E7EB]">
                        <th className="text-left py-2 px-3 text-[#696A72]">时间段</th>
                        <th className="text-left py-2 px-3 text-[#696A72]">活动内容</th>
                        <th className="text-left py-2 px-3 text-[#696A72]">活动地点</th>
                      </tr>
                    </thead>
                    <tbody>
              ...
              <div>
                <h3 className="font-semibold text-[#020617] mb-2">特殊需求</h3>
                <div className="flex flex-wrap gap-2">
                  {fpAnalysis2.lifestyleAnalysis.specialNeeds.map((need, i) => (
                    <span key={i} className="px-3 py-1 bg-[#0000EE]/10 rounded-full text-sm text-[#0000EE]">
                      {need}
                    </span>
                  ))}
                </div>
              ...
              <div>
                <h3 className="font-semibold text-[#020617] mb-2">收纳评估</h3>
                <div className="grid grid-cols-3 gap-4 mb-4">
                  <div className="text-center p-3 bg-gray-50 rounded-lg">
                    <p className="text-xs text-[#696A72] mb-1">当前容量</p>
                    <p className="font-semibold text-[#020617]">{fpAnalysis2.storageAssessment.currentCapacity}</p>
                  </div>
                  <div className="text-center p-3 bg-gray-50 rounded-lg">
                    <p className="text-xs text-[#696A72] mb-1">所需容量</p>
                    <p className="font-semibold text-[#020617]">{fpAnalysis2.storageAssessment.requiredCapacity}</p>
                  </div>
                ...
                <div>
                  <p className="text-sm text-[#696A72] mb-3">收纳解决方案：</p>
                  <div className="space-y-3">
                    {fpAnalysis2.storageAssessment.suggestions.map((s, i) => (
                      <div key={i} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                        <span className="w-6 h-6 rounded-full bg-[#0000EE] text-white text-xs flex items-center justify-center flex-shrink-0">
                          {i + 1}
                        </span>
                        <div>
                          <p className="font-medium text-[#020617]">{s.area}</p>
                          <p className="text-sm text-[#374151]">{s.solution}</p>
                          <p className="text-xs text-[#0000EE] mt-1">{s.estimatedCost}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div>
                <h3 className="font-semibold text-[#020617] mb-2">成长空间规划</h3>
                <div className="space-y-4">
                  {fpAnalysis2.growthPlanning.futureChanges.map((change, i) => (
                    <div key={i} className="flex items-start gap-3">
                      <div className="flex-shrink-0">
                        <span className="px-2 py-1 bg-[#0000EE]/10 rounded text-xs text-[#0000EE]">{change.timeframe}</span>
                      </div>
                      <div>
                        <p className="font-medium text-[#020617]">{change.change}</p>
              ...
              <div>
                <h3 className="font-semibold text-[#020617] mb-2">空间规划建议</h3>
                <div className="space-y-4">
                  {fpAnalysis2.spaceRecommendations.roomLayoutSuggestions.map((room, i) => (
                    <div key={i} className="p-4 bg-gray-50 rounded-lg">
                      <p className="font-semibold text-[#020617] mb-1">{room.room}</p>
                      <p className="text-sm text-[#374151] mb-2">{room.layout}</p>
                      <div className="flex flex-wrap gap-2">
                        {room.furniture.map((f, j) => (
                          <span key={j} className="px-2 py-1 bg-white rounded text-xs text-[#374151] border border-[#E5E7EB]">
                            {f}
                          </span>
                        ))}
                      </div>
          ...
          <Section title="装修决策师" icon={<Layers className="w-5 h-5 text-[#0000EE]" />}>
            <div className="space-y-6">
              <div>
                <h3 className="font-semibold text-[#020617] mb-3">装修优先级（分阶段实施）</h3>
                <PhaseTimeline phases={fpAnalysis3.renovationPriority.phases} />
              </div>

              <div>
                <h3 className="font-semibold text-[#020617] mb-3">费用估算明细</h3>
                <div className="bg-gray-50 rounded-lg p-4 mb-4">
                  <div className="flex items-center justify-between">
                    <span className="text-[#696A72]">总预算</span>
                    <span className="text-2xl font-bold text-[#0000EE]">
                  ...
                  {fpAnalysis3.costEstimation.breakdown.map((cat, i) => (
                    <div key={i} className="border border-[#E5E7EB] rounded-lg overflow-hidden">
                      <div className="px-4 py-3 bg-gray-50 flex items-center justify-between">
                        <span className="font-medium text-[#020617]">{cat.category}</span>
                        <span className="text-[#0000EE] font-semibold">{cat.amount.toLocaleString()}元</span>
                      </div>
                      <div className="p-4">
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="border-b border-gray-100">
                              <th className="text-left py-2 text-[#696A72]">项目</th>
                              <th className="text-left py-2 text-[#696A72]">单位</th>
                              <th className="text-right py-2 text-[#696A72]">数量</th>
                              <th className="text-right py-2 text-[#696A72]">单价</th>
                              <th className="text-right py-2 text-[#696A72]">合计</th>
                            </tr>
                          </thead>
                          <tbody>
                            {cat.items.map((item, j) => (
                              <tr key={j} className="border-b border-gray-50">
                                <td className="py-2 text-[#374151]">{item.name}</td>
                                <td className="py-2 text-[#374151]">{item.unit}</td>
                                <td className="py-2 text-right text-[#374151]">{item.quantity}</td>
                                <td className="py-2 text-right text-[#374151]">{item.unitPrice.toLocaleString()}元</td>
                                <td className="py-2 text-right font-medium text-[#020617]">{item.totalPrice.toLocaleString()}元</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                ...
                <div className="mt-4">
                  <p className="text-sm text-[#696A72] mb-2">省钱小贴士：</p>
                  <div className="flex flex-wrap gap-2">
                    {fpAnalysis3.costEstimation.costSavingTips.map((tip, i) => (
                      <span key={i} className="px-3 py-1 bg-green-50 rounded-full text-sm text-green-700">
                        {tip}
                      </span>
                    ))}
                  </div>
              ...
              <div>
                <h3 className="font-semibold text-[#020617] mb-3">避坑指南</h3>
                <div className="space-y-4">
                  {fpAnalysis3.pitfalls.map((cat, i) => (
                    <div key={i}>
                      <h4 className="font-medium text-[#020617] mb-2">{cat.category}</h4>
                      <div className="space-y-3">
                        {cat.items.map((item, j) => (
                          <div key={j} className="p-4 bg-gray-50 rounded-lg">
                            <div className="flex items-center gap-2 mb-2">
                              <AlertTriangle className="w-4 h-4 text-orange-500" />
                              <span className="font-medium text-[#020617]">{item.title}</span>
                              <RiskBadge level={item.riskLevel} />
                            </div>
                            <p className="text-sm text-[#374151] mb-2">{item.description}</p>
                            <div className="flex flex-wrap gap-4 text-sm">
                              <span className="text-[#34C759]">预防：{item.prevention}</span>
                              <span className="text-[#0000EE]">解决：{item.solution}</span>
                            </div>
                          </div>
                        ))}
                      </div>
              ...
              <div>
                <h3 className="font-semibold text-[#020617] mb-3">施工建议</h3>
                <div className="space-y-4">
                  {fpAnalysis3.constructionTips.map((tip, i) => (
                    <div key={i}>
                      <h4 className="font-medium text-[#020617] mb-2 flex items-center gap-2">
                        <Hammer className="w-4 h-4 text-[#0000EE]" />
                        {tip.area}
                      </h4>
                      <ul className="space-y-2">
                        {tip.tips.map((t, j) => (
              ...
              <div>
                <h3 className="font-semibold text-[#020617] mb-3">材料推荐</h3>
                <div className="grid md:grid-cols-2 gap-4">
                  {fpAnalysis3.materialSuggestions.map((mat, i) => (
                    <div key={i} className="border border-[#E5E7EB] rounded-lg overflow-hidden">
                      <div className="px-4 py-3 bg-gray-50">
                        <span className="font-medium text-[#020617]">{mat.area}</span>
                      </div>
                      <div className="p-4 space-y-3">
                        {mat.recommendations.map((rec, j) => (
                          <div key={j} className="flex items-start gap-3">
                            <div className="w-2 h-2 rounded-full bg-[#0000EE] mt-2" />
                            <div>
                              <p className="font-medium text-[#020617]">{rec.material}</p>
                              <p className="text-sm text-[#696A72]">{rec.brand}</p>
                              <p className="text-sm text-[#374151]">{rec.reason}</p>
                              <p className="text-xs text-[#0000EE]">{rec.priceRange}</p>
                            </div>
                          </div>
                        ))}
                      </div>
              ...
              <div>
                <h3 className="font-semibold text-[#020617] mb-3">时间规划</h3>
                <div className="bg-gray-50 rounded-lg p-4 mb-4">
                  <div className="flex items-center justify-between">
                    <span className="text-[#696A72]">总工期</span>
                    <span className="text-xl font-bold text-[#0000EE]">{fpAnalysis3.timeline.totalDuration}</span>
                  </div>
                </div>
                <div className="relative">
                  <div className="absolute left-4 top-0 bottom-0 w-0.5 bg-[#E5E7EB]" />
                  {fpAnalysis3.timeline.milestones.map((milestone, i) => (
                    <div key={i} className="flex gap-4 pb-6 last:pb-0">
                      <div className="relative">
                        <div className="w-8 h-8 rounded-full bg-white border-2 border-[#0000EE] flex items-center justify-center text-xs font-bold text-[#0000EE]">
                          {i + 1}
                        </div>
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-1">
                          <span className="font-medium text-[#020617]">{milestone.event}</span>
                          <span className="text-sm text-[#696A72]">{milestone.date}</span>
                        </div>
                        <p className="text-sm text-[#696A72]">负责人：{milestone.responsible}</p>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <Section title="AI 效果图" icon={<ImageIcon className="w-5 h-5 text-[#0000EE]" />}>
            <div className="grid md:grid-cols-2 gap-6">
              {fpAnalysis4.map((rendering, i) => (
                <div key={i} className="border border-[#E5E7EB] rounded-xl overflow-hidden">
                  <div className="relative aspect-[4/3]">
                    ...
                    <div className="absolute top-4 left-4">
                      <span className="px-3 py-1 bg-white/90 rounded-full text-sm font-medium text-[#020617]">
                        {rendering.area}
                      </span>
                    </div>
                    <button
                      ...
                      className="absolute top-4 right-4 w-10 h-10 bg-white/90 rounded-full flex items-center justify-center hover:bg-white transition-colors disabled:opacity-50"
                    >
                      {regeneratingArea === rendering.area ? (
                        <RefreshCw className="w-5 h-5 text-[#0000EE] animate-spin" />
                      ) : (
                        <RefreshCw className="w-5 h-5 text-[#0000EE]" />
                      )}
                    </button>
                  </div>
                  <div className="p-4">
                    <p className="text-sm text-[#696A72]">提示词：</p>
                    <p className="text-sm text-[#374151] mt-1 line-clamp-2">{rendering.prompt}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="flex items-center justify-center gap-4 mt-8 pt-8 border-t border-[#E5E7EB]">
            <button
              onClick={handleExportWord}
              className="flex items-center gap-2 px-6 py-3 border border-[#E5E7EB] rounded-full text-sm font-medium text-[#374151] hover:border-[#0000EE] hover:text-[#0000EE] transition-colors"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-6 py-3 bg-[#0000EE] text-white rounded-full text-sm font-medium hover:bg-[#145AFF] transition-colors"
            >
